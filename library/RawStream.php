...
class RawStream
{
	...
	private function file_stream($string)
	{
		...
		$image = ltrim(preg_replace('/Content-Type: (.*)[^\n\r]/', '', $match[3]));

		$path = sys_get_temp_dir().DIRECTORY_SEPARATOR.'php'.substr(sha1(rand()), 0, 6).'.tmp';
	...
	private function ready ($array) {
		$request_name = trim(strtoupper($_SERVER['REQUEST_METHOD']));
		$GLOBALS["_POST"] = [];
		$GLOBALS['_FILES'] = [];
		if (count($array['post'] > 0)) {
			foreach($array['post'] as $key => $value) {
		...
		if (count($array['file'] > 0)) {
			foreach($array['file'] as $key => $value) {
				foreach($value as $k => $v) {
					if (is_array($v)) {
						foreach($v as $kk => $vv) {
							$GLOBALS['_FILES'][$k][$kk] = $vv[0];
						}
					} else {
						$GLOBALS['_FILES'][$key] = $v;
					}
				}
			}
		}
